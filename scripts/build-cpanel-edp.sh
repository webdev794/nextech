#!/usr/bin/env bash
# First deployment bundle for https://testcaresortwork.co.in/edp/ — the user
# has already imported the local database to the live server by hand, so this
# ships a .env pre-filled with everything except the live DB credentials and
# mail password, which only the user has. No migrate, no seed.
set -euo pipefail

SRC=/d/edp
STAGE=/d/edp/.tmp/deploy/stage-edp
OUT=/d/edp/.tmp/deploy/edp-cpanel.zip
APP_KEY="base64:xQLD/+h7oLbe2Zb0rjLEcEbwyjgUcK8pV80xTgFKiLM="
SUBPATH=edp
...
# Stripe TEST keys — fill in your own, or leave blank to disable card payments.
...
NEXTECH - cPanel deployment bundle for /edp/ (db already imported)
====================================================================
...
2. Upload this zip INTO public_html/  and Extract it.
   You should end up with:  public_html/edp/index.php
   Everything lives inside public_html/edp/ - nothing is placed outside it.
3. Edit  public_html/edp/.env  and set:
      DB_DATABASE, DB_USERNAME, DB_PASSWORD   (the DB you already imported into)
      STRIPE_PUBLISHABLE_KEY / STRIPE_SECRET   (optional — leave CHANGE_ME to
         disable card payments; cash on delivery still works)
...
6. Open  https://testcaresortwork.co.in/edp/

Stripe (optional): dashboard > Webhooks > add endpoint
   https://testcaresortwork.co.in/edp/api/payments/stripe/webhook
   events: payment_intent.succeeded, payment_intent.payment_failed, payment_intent.canceled
   put its signing secret in .env as STRIPE_WEBHOOK_SECRET, then delete
   public_html/edp/bootstrap/cache/config.php  (it re-caches on next hit).
...
if command -v zip >/dev/null 2>&1; then
  ...
  php -r '
    ...
    $n = 0;
    foreach ($it as $f) {
      $rel = str_replace("\\", "/", substr($f->getPathname(), strlen($stage) + 1));
