...
// The shipping address can change until anything has left for the order.
const ADDRESS_CHANGE_STAGES = ['confirmed', 'packing', 'ready_for_delivery']
...
export default function Storefront() {
  ...
  const [addressChange, setAddressChange] = useState(null) // { order, name, line1, line2, city, state, postal_code }
  ...
  function openAddressChange(entry) {
    const a = entry.delivery_address ?? {}
    setOrdersMessage('')
    setAddressChange({ order: entry, name: a.name ?? '', line1: a.line1 ?? '', line2: a.line2 ?? '', city: a.city ?? '', state: a.state ?? '', postal_code: a.postal_code ?? '' })
  }
  ...
  async function submitAddressChange(event) {
    ...
    const token = localStorage.getItem('gdp_token')
    if (!token) { setOrdersMessage('Please sign in first.'); return }
    const { order, ...address } = addressChange
    try {
      const response = await fetch(`${API_URL}/orders/${order.id}/address-change`, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(address) })
      const data = await responseJson(response)
      if (!response.ok) throw new Error(data.message ?? Object.values(data.errors ?? {})[0]?.[0] ?? 'Could not send your request.')
      setOrders((current) => current.map((row) => row.id === order.id ? { ...row, address_changes: [data.data, ...(row.address_changes ?? [])] } : row))
      setAddressChange(null)
      setOrdersMessage('Address change requested — the seller will confirm before shipping.')
    } catch (error) { setOrdersMessage(error.message) }
  }
  ...
  return <><div className="app-shell">
    ...
    {addressChange && <div className="overlay" role="presentation" onClick={() => setAddressChange(null)}><div className="auth-modal" role="dialog" aria-modal="true" aria-labelledby="address-change-title" onClick={(event) => event.stopPropagation()}><button className="close-button" type="button" onClick={() => setAddressChange(null)} aria-label="Close">x</button><h2 id="address-change-title">Change shipping address</h2><p className="auth-intro">Order #{addressChange.order.id}. The seller confirms the new address before the order ships.</p><form onSubmit={submitAddressChange}>{addresses.length > 0 && <label className="address-picker">Use a saved address<select value="" onChange={(event) => { const a = addresses.find((x) => String(x.id) === event.target.value); if (a) setAddressChange((f) => ({ ...f, name: a.name || f.name, line1: a.line1 ?? '', line2: a.line2 ?? '', city: a.city ?? '', state: a.state ?? '', postal_code: a.postal_code ?? '' })) }}><option value="">Choose…</option>{addresses.map((a) => <option key={a.id} value={a.id}>{a.label} - {a.line1}, {a.city}</option>)}</select></label>}<input required placeholder="Full name" value={addressChange.name} onChange={(event) => setAddressChange({ ...addressChange, name: event.target.value })} /><input required placeholder="Street address" value={addressChange.line1} onChange={(event) => setAddressChange({ ...addressChange, line1: event.target.value })} /><input placeholder="Apartment, suite (optional)" value={addressChange.line2} onChange={(event) => setAddressChange({ ...addressChange, line2: event.target.value })} /><div className="form-row"><input required placeholder="City" value={addressChange.city} onChange={(event) => setAddressChange({ ...addressChange, city: event.target.value })} /><input required maxLength="60" placeholder="State" value={addressChange.state} onChange={(event) => setAddressChange({ ...addressChange, state: event.target.value })} /></div><input required maxLength="12" placeholder={marketProfile?.postal_label ?? 'ZIP code'} value={addressChange.postal_code} onChange={(event) => setAddressChange({ ...addressChange, postal_code: event.target.value })} /><button className="checkout-button" type="submit">Request change <span>&rarr;</span></button></form></div></div>}
    ...
    {supportView && <div className="overlay" role="presentation" onClick={() => setSupportView(null)}><div className="auth-modal support-modal" role="dialog" aria-modal="true" aria-labelledby="support-title" onClick={(event) => event.stopPropagation()}><button className="close-button" type="button" onClick={() => setSupportView(null)} aria-label="Close support">x</button><p className="eyebrow">We&rsquo;re here to help</p>
      {supportView === 'list' ? <>
        ...
        {supportView.seller_shop && <p className="auth-intro chat-privacy-note">A seller is in this chat. For your safety, don&rsquo;t share personal details — your address, phone number, email or any one-time code (OTP). Emails and phone numbers the seller sends are hidden, and yours are hidden from the seller.</p>}
        <ChatPhotoPicker photos={supportPhotos} onChange={setSupportPhotos} token={localStorage.getItem('gdp_token')} onError={setSupportMsg} disabled={supportBusy} />
